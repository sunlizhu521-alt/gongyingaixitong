...
app.post('/api/kcfx-library/inventory-turnover/export', async (req, res) => {
  try {
    ...
    const data = rows.map((row) => ({
      ...
      期间天数: row.periodDays,
      期初在库库存成本: row.openingOnHandInventoryCost,
      期末在库库存成本: row.closingOnHandInventoryCost,
      平均在库库存成本: row.averageOnHandInventoryCost,
      在库量存货周转天数: row.onHandInventoryTurnoverDays,
      期初在途库存成本: row.openingInTransitInventoryCost,
      期末在途库存成本: row.closingInTransitInventoryCost,
      平均在途库存成本: row.averageInTransitInventoryCost,
      在途量存货周转天数: row.inTransitInventoryTurnoverDays,
      期初未交付库存成本: row.openingUndeliveredInventoryCost,
      期末未交付库存成本: row.closingUndeliveredInventoryCost,
      平均未交付库存成本: row.averageUndeliveredInventoryCost,
      未交付存货周转天数: row.undeliveredTurnoverDays,
      期初库存合计成本: row.openingInventoryTotalCost,
      期末库存合计成本: row.closingInventoryTotalCost,
      平均库存合计成本: row.averageInventoryTotalCost,
      库存合计存货周转天数: row.inventoryTotalTurnoverDays,
