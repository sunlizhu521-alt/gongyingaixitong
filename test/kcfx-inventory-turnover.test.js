...
test('库存段筛选按所选段裁剪成本和未交付数量并保留销售分母', () => {
  ...
  assert.equal(undelivered.metrics.undeliveredTurnoverDays, 44.5);
...
test('菜单、独立权限、筛选器、查询和导出接口已接入', async () => {
  ...
  assert.match(page, /key: 'undeliveredTurnoverDays'[\s\S]*key: 'onHandQty', label: '在库量'[\s\S]*key: 'inTransitQty', label: '在途量'[\s\S]*key: 'inventoryTotalQty', label: '库存合计'/);
  ...
  assert.match(routes, /未交付周转天数: row\.undeliveredTurnoverDays,[\s\S]*在库量: row\.onHandQty,[\s\S]*在途量: row\.inTransitQty,[\s\S]*库存合计: row\.inventoryTotalQty/);
