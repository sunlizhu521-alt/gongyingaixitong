...
test('菜单、独立权限、筛选器、查询和导出接口已接入', async () => {
  ...
  assert.match(page, /inventorySegment', label: '库存段'[\s\S]*openingInventoryCost', label: '期初库存成本'[\s\S]*closingInventoryCost', label: '期末库存成本'[\s\S]*averageInventoryCost', label: '平均库存成本'[\s\S]*periodOperatingCost', label: '期间营业成本'[\s\S]*turnoverDays', label: '存货周转天数'/);
  ...
  assert.match(styles, /\.turnover-filter-toolbar \.turnover-end-month-input\s*\{[\s\S]*flex-basis:\s*267px/);
  assert.match(styles, /\.turnover-filter-toolbar \.turnover-end-month-input select\s*\{[\s\S]*width:\s*161px[\s\S]*padding:\s*0 30px 0 10px/);
