...
test('销售汇总分类和月份控件同步到月度销售、销售趋势及库龄分析', async () => {
  ...
  assert.match(loader, /salesRowsCacheVersion = 'v8'/);
  assert.match(filters, /targetFilter\.independentOptions/);
  assert.match(inventorySummary, /KCFX_INVENTORY_SUMMARY_VERSION = 17/);
  ...
  assert.match(utils, /仓库不为空但无法匹配或一级分类为空为“未匹配”/);
  ...
  assert.match(utils, /是否真实交易先按客户名称\+物料编码匹配销售部门/);
