...
test('销售汇总保留全部交易并按三个独立条件分类筛选', () => {
  ...
  assert.deepEqual(qualified.options.realTransactionStatus, ['真实交易', '非真实交易', '未匹配']);
  assert.deepEqual(qualified.options.nonInternalTransactionStatus, ['非内部交易', '内部交易', '未匹配']);
...
test('库存和销售汇总报表已分别接入菜单、页面、权限和受保护接口', async () => {
  ...
  assert.match(monthFilterSource, /availableYears\.includes\(displayYear\)[\s\S]*setDisplayYear/);
  assert.match(monthFilterSource, /月份加载中/);
