...
test('销售按年月和物料维度汇总应收数量与不含税金额', () => {
  ...
  const multiMonthRecords = sampleRecords();
  multiMonthRecords['sales-data'].rows.push({
    日期: '2026-02-10',
    客户名称: '客户A',
    物料编码: '1001',
    客户物料编码: '客户A1001',
  ...
  const multiMonthResult = queryInventorySummary(
    buildInventorySummaryCache(multiMonthRecords, 'saved-at'),
    { report: 'sales', filters: { salesMonth: ['2026-01', '2026-02'] } }
  );
  assert.deepEqual(multiMonthResult.options.salesMonth, ['2026-01', '2026-02']);
  assert.equal(multiMonthResult.metrics.salesQty, 9);
  assert.equal(multiMonthResult.metrics.salesAmount, 700);
});

test('销售汇总保留全部交易并按三个独立条件分类筛选', () => {
  const records = sampleRecords();
  records['dim-product'] = record([
    { 物料编码: '1001', SKU: 'SKU-1', 销售产品线: '产品线A', 一级分类: '成品', 金蝶名称: '正常成品' },
    { 物料编码: '1002', SKU: 'SKU-2', 销售产品线: '其他/配件', 一级分类: '成品', 金蝶名称: '其他配件' },
    { 物料编码: '1003', SKU: 'SKU-3', 销售产品线: '产品线A', 一级分类: '配件', 金蝶名称: '配件' },
  ...
  records['dim-warehouse'] = record([
    ...
    { 仓库名称: '集成仓', 一级仓库分类: '系统集成仓库' }
  ]);
  records['dim-store-name'] = record([
    { 匹配键: '客户A1001', 销售部门: '海外事业一部' },
    { 匹配键: '客户A1002', 销售部门: '海外事业一部' },
  ...
  records['sales-data'] = record([
    { 日期: '2026-01-01', 客户名称: '客户A', 物料编码: '1001', 客户物料编码: '客户A1001', 仓库: '销售仓', 应收数量: 1, '销售额-不含税': 10 },
    { 日期: '2026-01-02', 客户名称: '客户A', 物料编码: '1002', 客户物料编码: '客户A1002', 仓库: '集成仓', 应收数量: 2, '销售额-不含税': 20 },
    { 日期: '2026-01-03', 客户名称: '客户A', 物料编码: '1003', 客户物料编码: '客户A1003', 仓库: '销售仓', 应收数量: 3, '销售额-不含税': 30 },
    ...
    { 日期: '2026-01-06', 客户名称: '客户A', 物料编码: '1006', 客户物料编码: '客户A1006', 仓库: '销售仓', 应收数量: 6, '销售额-不含税': 60 },
    { 日期: '2026-01-07', 客户名称: '客户A', 物料编码: '9999', 客户物料编码: '客户A9999', 仓库: '', 应收数量: 7, '销售额-不含税': 70 }
  ]);
  ...
  const qualified = queryInventorySummary(cache, {
    report: 'sales',
    filters: {
      realTransactionStatus: ['是'],
      nonInternalTransactionStatus: ['是'],
      finishedGoodsStatus: ['是']
    }
  });
  assert.equal(qualified.pagination.totalRows, 1);
  assert.equal(qualified.rows[0].materialCode, '1001');
  assert.deepEqual(qualified.options.realTransactionStatus, ['是', '否', '未匹配']);
  assert.deepEqual(qualified.options.nonInternalTransactionStatus, ['是', '否', '未匹配']);
  assert.deepEqual(qualified.options.finishedGoodsStatus, ['是', '否', '未匹配']);

  assert.equal(queryInventorySummary(cache, {
    report: 'sales', filters: { realTransactionStatus: ['否'] }
  }).rows[0].materialCode, '1002');
  assert.equal(queryInventorySummary(cache, {
    report: 'sales', filters: { nonInternalTransactionStatus: ['否'] }
  }).rows[0].materialCode, '1003');
  assert.deepEqual(queryInventorySummary(cache, {
    report: 'sales', filters: { finishedGoodsStatus: ['否'] }
...
test('库存和销售汇总报表已分别接入菜单、页面、权限和受保护接口', async () => {
  const [constantsSource, mainSource, inventoryPageSource, salesPageSource, errorsPageSource, appSource, routeSource, filtersSource, monthFilterSource] = await Promise.all([
  ...
  assert.match(inventoryPageSource, /filters\.realTransactionStatus = \['是'\][\s\S]*filters\.nonInternalTransactionStatus = \['是'\][\s\S]*filters\.finishedGoodsStatus = \['是'\]/);
  assert.match(inventoryPageSource, /const SALES_SUMMARY_NOTE = \[/);
  assert.match(inventoryPageSource, /'统计口径：'/);
  assert.match(inventoryPageSource, /'1、是否真实交易按销售仓库匹配一级仓库分类/);
  assert.match(inventoryPageSource, /'2、是否非内部交易按客户名称\+物料编码匹配销售部门/);
